...
namespace CatanUtils {
	...
	{
		std::array<uint32_t, CatanUtils::Consts::NUMBER_OF_VALID_NODES - CatanUtils::Consts::NUMBER_OF_DESERT_RESOURCES> resource_numbers = {
		...
		for (auto i = 0; i < CatanUtils::Consts::NUMBER_OF_RESOURCES_IN_ROW; i++)
		{
			for (auto j = 0; j < CatanUtils::Consts::NUMBER_OF_RESOURCES_IN_COLUMN; j++)
			{
				if (is_valid_resource_index(i, j)) {
					if (resource_types.at(resource_type_index) == ResourceType::NONE) {
						m_resources.at(i).at(j) = std::make_shared<Resource>(ResourceType::NONE, CatanUtils::Consts::NUMBER_OF_DESERT_RESOURCE);
		...
		for (int i = 0; i < CatanUtils::Consts::NUMBER_OF_NODES_IN_COLUMN; i++)
		{
			for (int j = 0; j < CatanUtils::Consts::NUMBER_OF_NODES_IN_ROW; j++)
			{
	...
	std::string BasicBoard::to_string() const
	{
		...
		for (auto i = 0; i < CatanUtils::Consts::NUMBER_OF_RESOURCES_IN_COLUMN; i++) {
			for (auto j = 0; j < CatanUtils::Consts::NUMBER_OF_RESOURCES_IN_ROW; j++) {
				to_string << std::to_string(static_cast<uint32_t>(m_resources.at(i).at(j)->get_resource_type()))
					<< "," << m_resources.at(i).at(j)->get_resource_number() << ";";
			}
		}
		to_string << "\n";
		for (auto i = 0; i < CatanUtils::Consts::NUMBER_OF_NODES_IN_COLUMN; i++) {
			for (auto j = 0; j < CatanUtils::Consts::NUMBER_OF_NODES_IN_ROW; j++) {
				if (m_nodes.at(i).at(j)->get_structure() == nullptr) {
					...
					to_string << std::to_string(static_cast<uint32_t>(m_nodes.at(i).at(j)->get_structure_type())) << "," <<
						std::to_string(static_cast<uint32_t>(m_nodes.at(i).at(j)->get_structure()->get_player())) << ";";
				}

			}
		}
		to_string << "\n";
		for (auto i = 0; i < CatanUtils::Consts::NUMBER_OF_EDGES_IN_COLUMN; i++) {
			for (auto j = 0; j < CatanUtils::Consts::NUMBER_OF_EDGES_IN_ROW; j++) {
		...
		to_string << "\n" << std::to_string(m_robber_position.first) << "," << std::to_string(m_robber_position.second);

		return to_string.str();
	}

	const std::shared_ptr<Resource>& BasicBoard::get_resource(const uint32_t row_number, const uint32_t col_number) const 
	{
	...
	const std::shared_ptr<Edge>& BasicBoard::get_edge(const uint32_t row_number, const uint32_t col_number) const
	{
	...
	const std::shared_ptr<Node>& BasicBoard::get_node(const uint32_t row_number, const uint32_t col_number) const
	{
	...
	const std::array<std::array<std::shared_ptr<Node>, CatanUtils::Consts::NUMBER_OF_NODES_IN_ROW>, CatanUtils::Consts::NUMBER_OF_NODES_IN_COLUMN>& BasicBoard::get_nodes() const
	{
		return m_nodes;
	}

	const std::array<std::array<std::shared_ptr<Edge>, CatanUtils::Consts::NUMBER_OF_EDGES_IN_ROW>, CatanUtils::Consts::NUMBER_OF_EDGES_IN_COLUMN>& BasicBoard::get_edges() const
	{
		return m_edges;
	}

	const std::array<std::array<std::shared_ptr<Resource>, CatanUtils::Consts::NUMBER_OF_RESOURCES_IN_ROW>, CatanUtils::Consts::NUMBER_OF_RESOURCES_IN_COLUMN>& BasicBoard::get_resources() const
	{
		return m_resources;
	}	
	...
	void BasicBoard::set_robber_position(const std::pair<uint32_t, uint32_t> resource_number)
	{
		...
		m_resources.at(m_robber_position.first).at(m_robber_position.second).get()->set_is_robber_on(false);
	...
	void BasicBoard::create_edge(const uint32_t row_number, const uint32_t col_number, const PlayerType player_type)
	{
		if (!is_valid_edge_index(row_number, col_number)) {
	...
	void BasicBoard::create_settlement(const uint32_t row_number, const uint32_t col_number, const PlayerType player_type)
	{
		if (!is_valid_node_index(row_number, col_number)) {
	...
	std::vector<std::shared_ptr<Edge>> BasicBoard::get_edge_adjacent_edges(const uint32_t row_number, const uint32_t col_number) const
	{
		if (!is_valid_edge_index(row_number, col_number)) {
			throw InvalidEdgeIndex("The index is invlaid");
		}
	...
	std::vector<std::shared_ptr<Node>> BasicBoard::get_edge_adjacent_nodes(const uint32_t row_number, const uint32_t col_number) const
	{		
		if (!is_valid_edge_index(row_number, col_number)) {
			throw InvalidEdgeIndex("The index is invlaid");
		}
	...
	std::vector<std::shared_ptr<Edge>> BasicBoard::get_node_adjacent_edges(const uint32_t row_number, const uint32_t col_number) const
	{
		if (!is_valid_node_index(row_number, col_number)) {
			throw InvalidNodeIndex("The index is invlaid");
		}
	...
	std::vector<std::shared_ptr<Node>> BasicBoard::get_node_adjacent_nodes(const uint32_t row_number, const uint32_t col_number) const
	{
		if (!is_valid_node_index(row_number, col_number)) {
			throw InvalidNodeIndex("The index is invlaid");
		}
	...
	std::vector<std::shared_ptr<Resource>> BasicBoard::get_node_adjacent_resources(const uint32_t row_number, const uint32_t col_number) const
	{
		if (!is_valid_node_index(row_number, col_number)) {
			throw InvalidNodeIndex("The index is invlaid");
		}
	...
	std::vector<std::shared_ptr<Node>> BasicBoard::get_resource_adjacent_nodes(const uint32_t row_number, const uint32_t col_number) const
	{	
		if (!is_valid_resource_index(row_number, col_number)) {
			throw InvalidResourceIndex("The index is invalid");
		}
	...
	bool BasicBoard::is_valid_node_index(const uint32_t row_number, const uint32_t col_number) const {
		...
		if (row_number != 1 && row_number != 2 && col_number == CatanUtils::Consts::NUMBER_OF_NODES_IN_ROW - 1) {
			return false;
		}
